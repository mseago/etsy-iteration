...
/*average price of all items - the "price" field*/
function avgPriceOfAll(items){
  var sum = 0;
...
console.log(avgPriceOfAll(items));


/*show an array of items that cost between $14.00 and $18.00 USD*/
function arrOfItemsBtw14to18() {
  var itemsBtw14to18 = [];
  for (var obj of items) {
    if (obj.price > 14 && obj.price < 18){
    itemsBtw14to18.push(obj.title);
    }
  }
    return itemsBtw14to18;
}
console.log(arrOfItemsBtw14to18(items));

/*how find the item with a "GBP" currency code and print its name and price.
Please console.log the one you find.*/
function gbpCurrCodeItem() {
  for (var obj of items) {
    if (obj.currency_code === "GBP") {
      return ("item: " + obj.title + " for " + obj.price + " quid");
    }
  }
}
console.log(gbpCurrCodeItem(items));
...
console.log(itemsMadeOfWood(items));

/*find which items are made of eight or more materials.
Please console.log the ones you find.*/
function itemsWithEightOrMoreMats() {
  var eightOrMoreMaterials = [];
  for (var obj of items) {
    if (obj.materials.length > 7) {
      eightOrMoreMaterials.push(obj.title);
    }
  }
  return eightOrMoreMaterials;
}
console.log(itemsWithEightOrMoreMats(items));

// calculate how many items were made by their sellers
function itemsMadeBySeller() {
  var madeBySeller = [];
  for (var obj of items) {
    if (obj.who_made === "i_did") {
      madeBySeller.push(obj.title);
    }
  }
  return madeBySeller.length;
}
console.log(itemsMadeBySeller(items));
